...
function bestMonthLabel(dest) {
  const months = [...new Set((dest.bestSeason || []).flatMap(season => SEASON_MONTHS[season] || []))]
  return months.slice(0, 5).join(', ') || 'Oct-Feb'
}

function crowdLevelForToday(dest) {
  const now = new Date()
  const month = now.getMonth()
  const day = now.getDay()
  const season = [2, 3, 4, 5].includes(month) ? 'Summer' : [6, 7, 8].includes(month) ? 'Monsoon' : 'Winter'
  let score = 25
  score += (dest.bestSeason || []).includes(season) || (dest.bestSeason || []).includes('All Year') ? 24 : -6
  score += day === 0 || day === 6 ? 14 : 0
  ...
  if (score >= 75) return { label:'Very High', color:'#c0392b' }
  if (score >= 58) return { label:'High', color:'#e67e22' }
  if (score >= 38) return { label:'Moderate', color:'#f1c40f' }
    ...
    <div style={{ minHeight:'100vh', background:'#f0f4f8', fontFamily:'Poppins,sans-serif' }}>
      ...
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'30px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:'18px' }}>
          {dests.map(d => {
            const crowd = crowdLevelForToday(d)
            return (
            <div key={d._id} onClick={() => navigate(`/india/${d._id}`)}
              ...
              <div style={{ padding:'16px' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginBottom:'12px' }}>
                  <div style={{ background:'#f8fafc', border:'1px solid #e8eef5', borderRadius:'10px', padding:'8px' }}>
                    <div style={{ color:'#888', fontSize:'10px', fontWeight:'800', textTransform:'uppercase' }}>Best Time</div>
                    <div style={{ color:'#1e3a5f', fontSize:'12px', fontWeight:'800', marginTop:'2px' }}>{bestMonthLabel(d)}</div>
                  </div>
                  <div style={{ background:'#f8fafc', border:'1px solid #e8eef5', borderRadius:'10px', padding:'8px' }}>
                    <div style={{ color:'#888', fontSize:'10px', fontWeight:'800', textTransform:'uppercase' }}>Crowd Today</div>
                    <div style={{ color:crowd.color, fontSize:'12px', fontWeight:'900', marginTop:'2px' }}>{crowd.label}</div>
                  </div>
                </div>
